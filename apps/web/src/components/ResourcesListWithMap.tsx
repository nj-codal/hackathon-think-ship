...
export function ResourcesListWithMap({ resources, categories, category }: Props) {
    const [hoveredResourceId, setHoveredResourceId] = useState<string | undefined>(undefined)
        ...
        <div className="flex h-[calc(100vh-64px)] flex-col md:flex-row overflow-hidden bg-slate-50">
                    ...
                    <div className="mt-6 flex flex-wrap items-center gap-2">
                        ...
                        {categories.map((cat: any) => {
                            ...
                            return (
                                <Link 
                                    key={cat._id}
                                    href={`/resources?category=${cat.slug?.current}`} 
                                    className={`inline-flex items-center gap-1.5 rounded-full border-2 ${category === cat.slug?.current ? 'border-slate-900 bg-brand-600 text-white shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-900 hover:-translate-y-0.5'} px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition-all`}
                                >
                        ...
                        <div className="flex flex-col gap-4">
                            {resources.map((resource: any) => (
                                <ResourceCard 
                                    key={resource._id} 
                                    resource={resource} 
                                    isActive={hoveredResourceId === resource._id}
                                    onMouseEnter={() => setHoveredResourceId(resource._id)}
                                    onMouseLeave={() => setHoveredResourceId(undefined)}
                                />
                            ))}
                        </div>
                    ) : (
            ...
            <main className="relative hidden w-full h-[calc(100vh-64px)] bg-slate-100 p-4 md:block md:w-1/2 lg:w-[55%] xl:w-[60%]">
                <Suspense fallback={<div className="h-full w-full animate-pulse rounded-2xl bg-slate-200" />}>
                   <GoogleMapComponent resources={resources} activeResourceId={hoveredResourceId} />
                </Suspense>
            </main>
